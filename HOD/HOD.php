...
<header>
 ...
 <div style="margin-top: -28px;margin-left: 15px;">
  ...
  <div class="dropdown-content">
        <a href="Chart HOD.php">Hall Utilization</a>
        <a href="Change Password.php">Change Password</a>
        <a href="logout.php">Logout</a>
  </div>
</div>
<h1>Hearty Welcome! <br><?php  echo "$_SESSION[us2] - $_SESSION[us3]";?> Department Head</h1>
<div style="display: block;" id="da3" >
    <div class="tab1">
        ...
        <button class="tablinks" onclick="smenu(event, 'Confirmed')">Forwarded</button>
    ...
    $sql="SELECT * from log WHERE  HOD='none' AND Hall='$_SESSION[us3] Seminar Hall' and sdate>='$today'";
    ...
    $sql="SELECT * from log WHERE  HOD='none' AND Hall='$_SESSION[us3] Seminar Hall' and sdate>='$today' order by priority";
    $result=mysqli_query($con,$sql);
    if(mysqli_num_rows($result)>0)
    {
...
from log a, log b where a.HOD='none' AND a.Hall='$_SESSION[us3] Seminar Hall' and a.sdate>='$today' and a.sdate = b.sdate and a.stime=b.stime and a.edate=b.edate and a.etime=b.etime and a.Hall=b.Hall";
    $result=mysqli_query($con,$sql);
    if(mysqli_num_rows($result)>0)
    {
            echo "<div class='t1'><table><tr><th>Name</th><th>Department</th><th>Timing</th><th>Date</th><th>Timestamp</th><th>Priority</th><th>Click to View</th></tr>";
    ...
    $sql="select distinct a.id,a.Name, a.Department,a.stime,a.etime,a.sdate,a.edate,a.timestamp,a.priority 
from log a, log b where a.HOD='none' AND a.Hall='$_SESSION[us3] Seminar Hall' and a.sdate>='$today' and a.sdate = b.sdate and a.edate=b.edate and a.Hall=b.Hall";
    $result=mysqli_query($con,$sql);
    if(mysqli_num_rows($result)>0)
    {
            echo "<div class='t1'><table><tr><th>Name</th><th>Department</th><th>Timing</th><th>Date</th><th>Timestamp</th><th>Priority</th><th>Click to View</th></tr>";
    ...
    <div id="Confirmed" class="tabcontent">
        ...
        $sql="select * from log where HOD='Recommended' AND Hall='$_SESSION[us3] Seminar Hall' and sdate>='$today'";
        $result=mysqli_query($con,$sql);
        if(mysqli_num_rows($result)>0)
        {
                echo "<div class='t1'><table><tr><th>Name</th><th>Department</th><th>Priority</th><th>Purpose</th><th>Level</th><th>Date</th><th>Timing</th></tr>";
                while($row = mysqli_fetch_assoc($result)) {
                    $id2=$row["id"];
                    $sdate = $row["sdate"];
                    $edate = $row["edate"];
                    $start = date("d-m-Y", strtotime($sdate));
                    $end = date("d-m-Y", strtotime($edate));
                    echo "<tr><td> " . $row["Name"]. " </td><td> " . $row["Department"]. " </td><td> " . $row["priority"]. " </td><td> " . $row["Purpose"]. " </td><td>" . $row["level"]."</td><td>" . $start .' to<br>'. $end."</td><td>" . $row["stime"]. " to<br>".$row["etime"]."</td></tr>";
                }
                echo "</table></div>";
        }
        else
    ...
    <div id="Cancelled" class="tabcontent">
        ...
        $sql="select * from  log where HOD='Not Recommended' AND Hall='$_SESSION[us3] Seminar Hall' and sdate>='$today'";
        $result=mysqli_query($con,$sql);
        if(mysqli_num_rows($result)>0)
        {
        ...
        <input type='hidden' name='etime' value='$etime'>
                    <tr><td> " . $row["Name"]. " </td><td> " . $row["Department"]. " </td><td> " . $row["priority"]. " </td><td> " . $row["Purpose"]. " </td><td>" . $row["level"]."</td><td>" . $start .' to<br>'. $end."</td><td>" . $row["stime"]. " to<br>".$row["etime"]."</td><td><button class='actnbtn acptbtn' type='submit' NAME='BTN_ACN' value='accept2'>Recommend</button></td></tr></form>";
                }
                echo "</table></div>";
        }
        else
