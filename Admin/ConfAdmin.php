...
<div style="display: block;" id="da3" >
    ...
    $sql="SELECT * from conferrence WHERE  Admin='none' AND Hall='$Seminar' and sdate>='$today'";
    ...
    if(mysqli_num_rows($result)>0)
    {
            ...
            while($row = mysqli_fetch_assoc($result)) {
                ...
                echo "<form action='RecommendConf.php' method='post'><input type='hidden' name='id' value='$id1'><tr><td> " . $row["Name"]. " </td><td> " . $row["Department"]. " </td><td> " . $row["stime"].'<br>to '.$row["etime"] ." </td><td> " . $start .'<br>to '. $end."</td><td>".$row["timestamp"]."</td><td><button class='actnbtn acptbtn' type='submit' NAME='BTN_ACN' >View</button></td></tr></form>";
            }
            echo "</table></div>";
    }
    else
    ...
    $sql="SELECT * from conferrence WHERE  Admin='none' AND Hall='$Seminar' and sdate>='$today' order by priority";
    $result=mysqli_query($con,$sql);
    if(mysqli_num_rows($result)>0)
    {
            ...
            while($row = mysqli_fetch_assoc($result)) {
                ...
                $start = date("d-m-Y", strtotime($sdate));
                $end = date("d-m-Y", strtotime($edate));
                echo "<form action='RecommendConf.php' method='post'><input type='hidden' name='id' value='$id1'><tr><td> " . $row["Name"]. " </td><td> " . $row["Department"]. " </td><td> " . $row["stime"].'<br>to '.$row["etime"] ." </td><td> " . $start .'<br>to '. $end."</td><td>".$row["timestamp"]."</td><td>".$row["priority"]."</td><td><button class='actnbtn acptbtn' type='submit' NAME='BTN_ACN' >View</button></td></tr></form>";
            }
            echo "</table></div>";
    }
    else
...
from conferrence a, conferrence b where a.Admin='none' AND a.Hall='$Seminar' and a.sdate>='$today' and a.sdate = b.sdate and a.stime=b.stime and a.edate=b.edate and a.etime=b.etime and a.Hall=b.Hall";
    $result=mysqli_query($con,$sql);
    if(mysqli_num_rows($result)>0)
    {
            echo "<div class='t1'><table><tr><th>Name</th><th>Department</th><th>Timing</th><th>Date</th><th>Timestamp</th><th>Priority</th><th>Click to View</th></tr>";
    ...
    $sql="select distinct a.id,a.Name, a.Department,a.stime,a.etime,a.sdate,a.edate,a.timestamp,a.priority 
from conferrence a, conferrence b where a.Admin='none' AND a.Hall='$Seminar' and a.sdate>='$today' and a.sdate = b.sdate and a.edate=b.edate and a.Hall=b.Hall";
    $result=mysqli_query($con,$sql);
    if(mysqli_num_rows($result)>0)
    {
            echo "<div class='t1'><table><tr><th>Name</th><th>Department</th><th>Timing</th><th>Date</th><th>Timestamp</th><th>Priority</th><th>Click to View</th></tr>";
    ...
    <div id="Confirmed" class="tabcontent">
        ...
        $sql="select * from conferrence where Admin='Approved' AND Hall='$Seminar' and sdate>='$today'";
        $result=mysqli_query($con,$sql);
        if(mysqli_num_rows($result)>0)
        {
                ...
                while($row = mysqli_fetch_assoc($result)) {
                    ...
                    $start = date("d-m-Y", strtotime($sdate));
                    $end = date("d-m-Y", strtotime($edate));
                    echo "<form action='Not RecommendConf.php' method='post'><input type='hidden' name='id' value='$id2'><input type='hidden' name='sdate' value='$sdate'>
    ...
    <div id="Cancelled" class="tabcontent">
        ...
        $sql="select * from  conferrence where Admin='Not Approved' AND Hall='$Seminar' and sdate>='$today'";
        $result=mysqli_query($con,$sql);
        if(mysqli_num_rows($result)>0)
        {
                echo "<div class='t1'><table><tr><th>Name</th><th>Department</th><th>Priority</th><th>Purpose</th><th>Level</th><th>Date</th><th>Timing</th><th>Action</th></tr>";
